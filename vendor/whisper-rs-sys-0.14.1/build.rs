...
fn strip_bindgen_layout_asserts(bindings_path: &PathBuf) {
    let content = std::fs::read_to_string(bindings_path)
        .unwrap_or_else(|e| panic!("Failed to read {}: {}", bindings_path.display(), e));

    let marker = "#[allow(clippy::unnecessary_operation, clippy::identity_op)]";
    ...
    while let Some(line) = lines.next() {
        if line.trim() == marker {
            if let Some(next_line) = lines.peek() {
                if next_line.trim() == "const _: () = {" {
                    // Drop bindgen layout checks because they are host-libc-specific
                    // and can fail to compile on other targets.
                    lines.next();
    ...
    std::fs::write(bindings_path, output)
...
fn main() {
    ...
    // Use bundled bindings unconditionally to avoid host-specific bindgen layout failures.
    let bindings_out = out.join("bindings.rs");
    let _: u64 = std::fs::copy("src/bindings.rs", &bindings_out)
        .expect("Failed to copy bindings.rs");
    strip_bindgen_layout_asserts(&bindings_out);
